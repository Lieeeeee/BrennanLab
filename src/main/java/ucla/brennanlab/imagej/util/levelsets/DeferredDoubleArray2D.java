...
public class DeferredDoubleArray2D {
	...
	public DeferredDoubleArray2D(final int xdim, final int ydim,
			final int tilesize, final double defaultval) {
	 ...
	 *            The Y index
	 * @param create
	 *            Determines whether the tile should be created if it has not
	 *            been allocated yet.
	...
	protected final Object createTile(final int tilesize) {
		final double[][] tile = new double[tilesize][tilesize];
	...
	public final double get(final int x, final int y) {
		final double[][] tile = (double[][]) getTile(x, y, false);
	...
	public final void set(final int x, final int y, final double value) {
		final double[][] tile = (double[][]) getTile(x, y, true);

...
/**
 * CSD Wavefront segmentation from difference image sequences. This plugin
 * requires Apache commons math and random modules. This plugin is more
 * restrictive than the other plugin in that it does not really compute the
 * prior speed field. Instead it uses a more simplified speed model where there
 * is time correlation but no spatial effects, so that the entire interface
 * travels with a single speed
 * 
 * @author Josh Chang
 * 
 */
public class csdWavefrontMotionSegmentation implements PlugInFilter {
    ImagePlus imp;
    int timesteps;
    int slices;
    int firstslice;
	...
	// ImagePlus gSegment = new ImagePlus("Segment_results");
	 ...
	 * 
	 * shapeKernelDensityEstimation nullShapeKernelPrior = new
	 * shapeKernelDensityEstimation( width, height);
	 * 
	 *GenericLevelSet nullShape = nullShapeKernelPrior.shapes.get(0);
	 * 
	 *****************************************************************/
	...
	while (currentSlice <= lastCSDslice) {
		...
		GenericLevelSet prevLS =new GenericLevelSet(gcs.returnMask());
		...
		ArrayList<GenericLevelSet> positions = new ArrayList<GenericLevelSet>(
			speedSamples);
		...
		GenericLevelSet meanNext = new GenericLevelSet(
			csdWavefrontMotionSegmentation.deepCopy(prevLS
				.getSignedDistance()));
		meanNext.advectUniformSpeed(this.priorSpeedMean, 1 / this.freq);

		Roi nextRoi = (RoiImageLevelSet.genericToRoi(meanNext))
			.getRoi(false);
		imp.setRoi(nextRoi);
		...
		for (int i = 0; i < this.speedSamples; i++) {
		    ...
		    positions.add(new GenericLevelSet(
			    csdWavefrontMotionSegmentation.deepCopy(prevLS
				    .getSignedDistance())));
		    effectiveWeights[i] = 1;// 
		    positions.get(i).advectUniformSpeed(speed, 1 / this.freq);

		    Roi pred = (Roi) RoiImageLevelSet.genericToRoi(
			    (GenericLevelSet) positions.get(i)).getRoi(true);
		...
		shapeKernelDensityEstimation skde = new shapeKernelDensityEstimation(
			effectiveWeights, positions);

		skde.setAlpha(alpha);
		skde.estimateSigmaSquared();
		IJ.log("Prediction distance variance " + skde.sigmaSquared);
		...
		float tempE;
		long endTime;
		ImageStats s = new ImageStats(5, 2, 0, 1);
		s.updateNarrowBandStats(currentImageProcessor, meanNext
			.getMask(), this.innerBandWidth);
		IJ.log("inner intensity " + s.innerinnermean + "+-"
			+ s.innerinnersd + " outer " + s.outermean + "+-"
			+ s.outersd);
		gcs = new graphCutSegmenter(currentImageProcessor);
		gcs.setLengthPenalty(this.lengthPenalty);
		...
		try {
		     ...
		     * MM iterations
		     */
		    s.updateNarrowBandStats(currentImageProcessor, gcs
			    .returnMask(), this.innerBandWidth);
		    gcs.setEdgeWeights(skde, gcs.getLevelSet());
		    long currentTime = startTime;
		    int i = 1;
		    /*
		     * Ever 5 iterations or so, let's show the progress!!
		     */
			...
			// if (i % 3 == 0) {
			try {
			    Roi currentProgress = (Roi) (RoiImageLevelSet
				    .genericToRoi(gcs.getLevelSet())).getRoi(
				    false).clone();
		...
		if (segmentedRoi == null) {
		    segmentedRoi = RoiImageLevelSet.genericToRoi(meanNext)
			    .getRoi(false);
		}
		if (segmentedRoi != null) {
		...
		GenericLevelSet currentPosition = gcs.getLevelSet();
		// BLEH
		
		
		// calculate new speed? update?
		this.maxAposteriorSpd.addArrival(currentPosition);

		// Use Kriging2DLattice TODO
		// NOW LET'S USE THE KRIGING SPEED FIELD!!!!
		
		
		
		this.priorSpeedMean = 0.9 * this.maxAposteriorSpd.latestSpeed
			+ 0.1 * this.priorSpeedMean;
		if (s.outersd > s.innerinnermean) {
		    IJ
		...
		if (this.useManualInitializationPrior) {
		    ...
		    GenericLevelSet manualLS = (new RoiImageLevelSet(
			    this.userDrawnRoi, width, height));
		    ArrayList<GenericLevelSet> userDefinedShapes = new ArrayList<GenericLevelSet>(
			    1);
		    userDefinedShapes.add(manualLS);
		    double[] weights = new double[1];
		    weights[0] = 1;
		    shapeKernelDensityEstimation userKDE = new shapeKernelDensityEstimation(
			    weights, userDefinedShapes);
		    userKDE.setAlpha(2);
		    userKDE.sigmaSquared = 1;
		    ImageStats s = new ImageStats();
		    s.updateStats(currentImageProcessor, manualLS.getMask());
		    IJ.log("User Drawn: inner intensity " + s.innerinnermean
			    + "+-" + s.innerinnersd + " outer " + s.outermean
			    + "+-" + s.outersd);
	...
	gcs.setLengthPenalty(this.initialLengthPenalty);
	int i = 1;
	gcs.setEdgeWeights();
	while (i++ < iterations) {
	    gcs.setNodeWeights(s);
	    IJ.log("Energy " + gcs.relaxEnergy());
	    s.updateStats(ip, gcs.returnMask());
	}
	return gcs;
